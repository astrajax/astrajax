...
export interface DraftTruthWriteInput {
  ...
  /**
   * Default true: a draft without a live brain has no destination. Only the
   * memory/demo store may relax it, and only because nothing is written.
   */
  requireBrainRegistryLink?: boolean;
}

/**
 * The write map. Every Draft Brain Truth payload keys on these field IDs, so a
 * column rename cannot break capture the way the 17 Aug 2026 rename did.
 */
export const DRAFT_TRUTH_FIELD_IDS = BRAIN_WORKSHOP_DRAFT_TRUTH_FIELDS;

/**
 * Field *names* for the same columns. Reads may still arrive name-keyed (Airtable
 * defaults to names unless `returnFieldsByFieldId` is set), and `filterByFormula`
 * and `sort` accept names only. Never build a write payload from this map.
 */
export const DRAFT_TRUTH_FIELD_NAMES = {
  ...
  systemBrainName: "System Brain Name",
  systemBrainSlug: "System Brain Slug",
...
export function buildDraftTruthCreateFields(
  ...
  if (input.requireBrainRegistryLink !== false) {
    if (!brainRegistryRecordId) {
      throw new Error(
        `Draft Brain Truth requires a live Brain Registry link for ${brainSlug}. A brain slug is a label, not a destination.`,
      );
    }
    if (!LIVE_RECORD_ID.test(brainRegistryRecordId)) {
      throw new Error(
        `Brain Registry expects a live record ID, not ${brainRegistryRecordId}.`,
      );
    }
  }
  ...
  if (brainRegistryRecordId) {
    fields[DRAFT_TRUTH_FIELD_IDS.brainRegistry] = [brainRegistryRecordId];
  }
  ...
  if (input.contextAmendmentVersionRecordIds?.length) {
    fields[DRAFT_TRUTH_FIELD_IDS.contextAmendmentVersions] = input.contextAmendmentVersionRecordIds;
  }
...
/**
 * The single door. Resolve the live brain, build an ID-keyed payload, POST it.
 * Every website capture path calls this — file mining, the Receiving Wall, intake
 * routing, demo seeding, chat proposals. There is no second door on field names.
 */
export async function createDraftTruthRecord(
  baseId: string,
  token: string,
  input: Omit<DraftTruthWriteInput, "brainRegistryRecordId"> & {
    brainRegistryRecordId?: string;
    tableId?: string;
  },
): Promise<{ recordId: string; fields: Record<string, unknown> }> {
  const brainRegistryRecordId =
    input.brainRegistryRecordId ??
    (await resolveBrainRegistryRecordId(baseId, token, input.brainSlug)) ??
    undefined;

  const fields = buildDraftTruthCreateFields({ ...input, brainRegistryRecordId });

  const created = await airtableCreate(
    baseId,
    input.tableId ?? BRAIN_WORKSHOP_TABLES.draftBrainTruth,
    token,
    fields,
    { returnFieldsByFieldId: true },
  );

  return { recordId: created.id, fields };
}
...
/**
 * Read one Draft Brain Truth cell whichever way Airtable keyed the response.
 * `returnFieldsByFieldId` gives IDs; older call sites and fixtures give names.
 * Agent text also accepts the retired name `Canonical Text` so pre-rename
 * fixtures and partial responses still resolve.
 */
export function readDraftTruthField(
  fields: Record<string, unknown>,
  key: keyof typeof DRAFT_TRUTH_FIELD_NAMES,
): unknown {
  const byId = fields[DRAFT_TRUTH_FIELD_IDS[key]];
  if (byId !== undefined) return byId;
  const byName = fields[DRAFT_TRUTH_FIELD_NAMES[key]];
  if (byName !== undefined) return byName;
  if (key === "canonicalTextForAgents" && fields["Canonical Text"] !== undefined) {
    return fields["Canonical Text"];
  }
  return undefined;
}

/** Trimmed string read of one Draft cell, by field ID first. */
export function readDraftTruthText(
  fields: Record<string, unknown>,
  key: keyof typeof DRAFT_TRUTH_FIELD_NAMES,
): string {
  const value = readDraftTruthField(fields, key);
