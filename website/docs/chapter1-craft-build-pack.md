...
- **CRITICAL — Agent plates on books:** Whenever an agent still or video sits on a Living Folio / book page, the plate must dissolve into the parchment on **all four sides** with the same papery deckle (no hard crop / ruler line through the figure, especially at the bottom). Welcome and other body copy sit *below* that dissolve — they are not the plate edge. Curated MP4s stay the media. Shipped stack: one painted SVG mask on `.study-stage__clive-spot` — interaction `/brand/system-assets/folio/masks/clive-folio-mask-interaction.svg`, teaching `clive-folio-mask-teaching.svg` (`globals.css`); nested feather/poster/layer masks stay off (not soft `radial-gradient` alone). Teaching = left page; interaction = right top-third. Cursor mirror: `.cursor/rules/folio-agent-plate-deckle.mdc`.
