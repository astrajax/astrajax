...
- **CRITICAL — Agent plates on books:** Whenever an agent still or video sits on a Living Folio / book page, the plate must dissolve into the parchment on **all four sides** with the same papery deckle (no hard crop / ruler line through the figure, especially at the bottom). Welcome and other body copy sit *below* that dissolve — they are not the plate edge. Curated MP4s stay the media; REF supplies the **border only**. Stack: `.study-stage__clive-plate` clips with luminance matte `https://cvu4l5kwtlocutgd.public.blob.vercel-storage.com/folio/masks/clive-folio-deckle-v9-4k.png` (`mask-mode: luminance`, 4096×3020); sibling `.study-stage__clive-ink` paints `clive-folio-deckle-ink-v9-4k.png` outside that clip (nested feather/poster/layer masks stay off — not soft `radial-gradient` alone). Keep the plate at the matte’s native aspect (`1024 / 755`). Teaching = left page (v9-4k approved, aspect `1024 / 755`); interaction = right top-third with its own shorter pair (`clive-folio-deckle-interaction-v9-4k.png` + ink, 4096×2518, aspect `4096 / 2518`) under `[data-folio-state="interaction"]` — look not declared final. Cursor mirror: `.cursor/rules/folio-agent-plate-deckle.mdc`.
