# AstraJax — Investor One-Pager

**Status:** External-facing investor narrative.  
**Audience:** Angels, pre-seed/seed investors, strategic partners.  
**Companion docs:** `docs/business/positioning.md` (product), `docs/business/proof.md` (evidence), `docs/business/internal-brief.md` (claim control).  
**Owner:** Matthew.  
**Last updated:** 23 June 2026.

---

## The Thesis

**AstraJax is building the company of the future now: small, creative, commercially sharp teams directing AI agents instead of waiting behind traditional software delivery queues.**

The old software company assumed humans do the technical labour and domain experts submit tickets. That model is breaking. AI can build, test, monitor, and improve software at a pace no conventional dev team can match on cost or iteration speed.

AstraJax is not betting on that shift from the sidelines. We are **living it**: a non-technical founder shipped production software with AI, governed a real commercial operation with it, and is now productising the operating model that made it work.

> **We are building for the post-developer-bottleneck era.**

---

## What We Are Building

Two things, one flywheel.

**1. The product — Adoption OS for AI agents.**  
AstraJax helps domain experts — not developers — design, run, and improve agent fleets their teams actually use. Reason → challenge → human approval → dispatch → runtime execution → coaching → brain improves. Full detail: `docs/business/one-pager.md`.

**2. The company — an AI-native operating model.**  
We are not scaling as a traditional dev-heavy software house. We hire high-judgement, AI-curious operators, creatives, and commercial leaders — people with taste, emotional intelligence, and domain context — and coach them to direct AI build systems the way our founder already does. Agents take the sludge. Humans keep judgement, client context, and approval.

The product and the company prove each other. Clients buy adoption. Investors buy a new labour model with production evidence behind it.

---

## Why Now

- **Agent building is solved. Adoption is not.** Most rollouts stall after the demo. The gap is context, trust, governance, and maintenance — not another builder.
- **The labour model is shifting.** From GPT-4.1 vibe-coding to frontier models that reason, self-correct, and execute — the gap between intent and shipped software is collapsing. The scarce skills are judgement, taste, and knowing what good looks like.
- **The market still sells to builders.** Non-technical domain experts are told they can build, then left inside tools built for developers. AstraJax sits upstream: adoption, context brains, human gates, coaching loops.
- **Ecosystem tailwinds.** Airtable MVP, HyperAgent Founding 500, Airspace LA keynote — vendors are publicly validating the "domain expert as architect" thesis. We are ahead of the curve, not chasing it.

---

## Proof It Works (Not a Pitch Deck)

Matthew Hopkinson is a non-technical commercial leader who has **never handwritten a line of code**. Working with AI on top of clean operational data, he built a production operating layer for **Butternut Box Direct Sales** — a real commercial function with a real team and real adoption pressure.

**Canonical numbers:**

| Signal | Proof |
|--------|-------|
| Scale | **~£8.1m** Direct Sales channel; 3 P&Ls; ~15 office FTE + ~90 field salespeople |
| Sequencing | **~12 months** boring layer first (data, workflows, architecture) → **~556 TS/React files** in ~1 month → **14 agents in ~3 weeks** |
| Outcomes | **~£180k/year** travel saving; **~3,000 hours/year** operational capacity at scale |
| Adoption | Characterful agent fleet used by a non-technical team; training hub, sandboxes, engagement leaderboard, human approval everywhere |
| External validation | Airtable **Airspace LA** keynote; **Airtable MVP**; **HyperAgent Founding 500** |

The proof is not founder magic. It is **stubborn execution on clean foundations**: data first, bounded agents, human gates, audit trails, and a governed report → fix → ship → sign-off loop running in production today.

**Read this honestly.** This is **one production proof, built internally** — Matthew was a commercial leader running a ~£8.1m channel *inside* a unicorn-scale company, not AstraJax landing external clients yet. Butternut proves the *thesis*. The *business* — repeatability across people and paying clients — is what we are building and raising for now. We would rather you hear that from us than find it in diligence.

---

## How The Company Runs (We Eat Our Own Cooking)

AstraJax operates on the same pattern it sells:

```text
Reason → Challenge → Human decides → Doc acts → Review → Improve
```

| Role | Who | What they do |
|------|-----|--------------|
| **Founder / commercial architect** | Matthew | Direction, client context, adoption culture, approval gates |
| **Doc layer** | Cursor + agent fleet | Turns approved briefs into code, docs, demos, deploys |
| **Creative director** | Tara-Lee | Visual identity, agent personas, adoption infrastructure |
| **Story director** | David | Narrative, pacing, human trust through change |
| **Runtime** | HyperAgent | Executes bounded agents with human-in-the-loop everywhere |

Quality, security, architecture, and production risk are not owned by job titles. They live in **prompts, review gates, red-team agents, deployment checks, logs, and human approval loops** — encoded into the operating system, not trapped in one developer's head.

We trailblaze. We also govern. Disruption without rigour is just noise.

---

## Why This Scales Lean

**Lower fixed cost.** No conventional engineering payroll to scale before revenue. High-judgement generalists + AI agents + specialist review where risk demands it.

**Faster iteration.** People closest to the work shape systems without a dev queue. Feedback → fix → ship loops measured in hours, not quarters.

**Repeatable method.** The founder proof is step one. AstraJax productises the coaching path: user brain → business brain → fleet → maturity → efficiency credit. Each client brain makes the method sharper.

**Better sales motion.** AI-curious operators and creatives adopt and sell the method better than traditional builders selling to people who fear AI.

**Dual revenue path.** Platform-led adoption OS + premium partnership for teams that want hands-on architecture and champion training.

---

## The Wedge (How We Land And Grow)

We do not sell "AI transformation." We sell a sequence with a low-friction front door.

```text
Adoption OS Audit  →  Brain & Fleet Sprint  →  Partnership / Platform
(diagnostic, paid)    (first brain + fleet)     (ongoing operating layer)
```

- **Who we chase first.** Commercial or operations-heavy teams (£5m–£100m revenue) where one or two operators hold too much context in their heads, earlier AI experiments stalled after the demo, and a commercial leader — not an IT function — owns the budget. Founder-led SMEs, direct-sales and field operations, multi-site service businesses.
- **Why us, not a consultancy.** The Audit leaves them with a working brain and a bounded fleet, not a slide deck. The method compounds: every client brain we build sharpens the coaching path and the product.
- **Why it becomes a category, not a boutique.** The hands-on partnership funds and de-risks the platform. As the coaching path productises, delivery shifts from founder-led to method-led to self-serve — the consultancy is *how it is delivered today*, not *what it is*.

---

## The Moat

Not the stack. Not "we use HyperAgent."

- **Adoption method** — context discipline, brain maturity, human judgement layer, coaching loops
- **Founder proof at scale** — production, not slideware; vendor-validated publicly
- **Operating model as product** — the company shape IS the thesis investors are buying
- **Personality as adoption infrastructure** — characters, story, and warmth that make non-technical teams actually use the system
- **Portable context brains** — tool-agnostic by design; client truth stays portable across runtimes

---

## What We Are Not

- A generic AI consultancy
- An Airtable build shop or chatbot installer
- "No developers ever" recklessness — we are **anti-bottleneck**, not anti-skill
- A pitch without production evidence

---

## The Unknowns We Are Buying Down

We would rather name these than have you guess. The bet is attacking them deliberately.

- **Repeatability beyond the founder.** Only Matthew has shipped to the founder bar so far. The core experiment of the next 12 months: can an AI-curious operator be coached to that bar, and how fast? This is the single biggest unknown — and the main thing your capital de-risks.
- **External vs internal proof.** Butternut was built as an internal operator. First paying external clients are in progress, not closed. We are converting founder proof into client proof.
- **Platform dependency.** We build on Airtable, HyperAgent, Cursor, and Vercel. Mitigation is portable, human-approved context brains that move across runtimes — but a platform shipping "adoption" natively, or changing terms, is a live risk we actively watch.
- **Defensibility timing.** The method is the moat; early on, parts are copyable. The race is to compound context discipline, client brains, and reputation faster than the category fills in.

A founder who can name these is a safer bet than one who pretends they do not exist.

---

## The Ask

**Raising:** a pre-seed round to convert founder proof into repeatable, client-proven traction.

**What your capital unlocks (next ~18 months):**

- **Coach the first cohort** of AI-curious operators/creatives to the founder bar — turning N=1 into the first evidence of repeatability.
- **Land the first paying external clients** through the Audit → Sprint wedge, with case studies that stand on their own without Butternut.
- **Productise the coaching path** (user brain → business brain → fleet → maturity) from founder-led delivery toward method-led delivery.

**What will be proven by the next round:** that someone other than the founder can deliver the result, for a paying client, repeatably — the inflection from "remarkable founder" to "scalable company."

*(Round size, structure, and use-of-funds breakdown available on request.)*

---

## Stage and Next Steps

**Now:** Core narrative, Butternut case study, live AstraJax OS demo, ecosystem relationships, warm outreach to first serious prospects.

**The opportunity for the right investor:** Back a founder who has already proven the thesis inside a unicorn-scale company — running a ~£8.1m commercial channel with zero handwritten code — and is now building the company that turns that method into a category.

---

## One-Line Close

> **HyperAgent makes powerful agents possible. AstraJax makes them adoptable — and builds the AI-native company that proves it.**

**Matthew Hopkinson** · Founder · AstraJax  
**AstraJax. AI that actually gets used.**
