...
**Incubation mode (Chapter 1, 20 Aug 2026):** not a second Trusted base. Early test-user filings share this Brain Truth table, tagged by Brain Theme. Volume cap is an application constant (100,000 Canonical Text characters primary; 100 rows watched; warn at 80k / 80 rows) — not Registry fields. Signed schema job: `scripts/ruth/jobs/incubation-mode-chapter1-truth.json`.
...
**Incubation Scope option (live 23 Aug 2026, add-only; do not retire legacy):**  
`read:brain-truth:incubation` (choice id `selyGCBWthBpJPfWc`)
...
**Live readback 23 Aug 2026 (Matthew UI):** Scope `read:brain-truth:incubation` is on Chapter 1 Brain Truth. Category is **partial** — live new choices are Definition, Goals (signed name was **Goals & Priorities**), Workflow. Still missing: Data & Metrics, Rules & Guardrails, Knowledge, Examples & Edge Cases, Open Questions, Business Context, Adjacent Functions. All seven legacy Category IDs and all four prior Scope IDs remain. Do **not** retire legacy options. Remaining Category add is still Matthew UI or the signed Ruth pen (`AIRTABLE_BUILD_TARGET_WRITE` on `app6tjzzG0L0lOeVb`). Estate map: `estate-map:chapter1-brain-truth:scope-incubation:2026-08-23` Applied; `estate-map:chapter1-brain-truth:category-universal:2026-08-23` Pending.
