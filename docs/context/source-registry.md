...
| `SRC-AJ-BRAIN-IDS` | `website/src/lib/brains/airtable-ids.ts` | Code ID map | Matthew | Live implementation | Current | Live Chapter 1 Airtable base, table, and field IDs; Draft dual-text + Brain Registry + Projects maps (17 Aug 2026); website Draft/Trusted writes key on field IDs; `BRAIN_WORKSHOP_SOURCE_DOCUMENTS_*` (29 Jun 2026); Clive + Clive's Man + Pam + Doc spines Approved-Canonical 27 Jun 2026; Kathryn Goodchild Agent base + Persona Config (11 Aug 2026) |
...
| `SRC-AJ-DRAFT-TRUTH-WRITE` | `docs/initiatives/draft-truth-platform-doc-handoff.md`; `website/src/lib/brains/draft-truth-write.ts`; `website/src/lib/brains/trusted-truth-write.ts`; `website/src/lib/brains/airtable-field-id-writes.test.ts` | Working brief / Website write helpers | Matthew | Working current (branch `doc/draft-truth-field-id-writes`, not merged 17 Aug 2026) | Current | Website Draft door: field-ID writes, dual text, required Brain Registry; Trusted promote dual-register + blank-only human backfill; wide field-name regression test |
