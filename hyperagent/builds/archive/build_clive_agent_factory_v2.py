#!/usr/bin/env python3
"""Build Clive Agent Factory V2 — Cursor-native agent + skill + build pack.

V2 changes (red-team driven):
- Cursor-native. Factory lives where agents are actually built. No Hyperagent
  agent export (the repo-roster script cannot run in Hyperagent's VM).
- Two-phase autonomy: read-only propose -> explicit Matthew approval -> Factory
  writes versioned artifacts in-repo.
- Risk tiering (Low/Medium/High) gates eval depth, edit-safety, and an
  independent Opus 4.7 red-team pass for High-risk agents.
- Roster check has a defined degraded path (repo is source of truth).
- Removed the broken Context Intake write path.
"""
...
SKILL_BODY = f"""# clive-agent-factory
...
- `agents/registry/<platform>/<family>/<short-name>/build-pack-v<n>.md` — human-readable
...
## Hyperagent platform preload (mandatory on every Factory run)

Before any roster check, interview, draft, or build, Factory must read these
files:
...
This preload matters even when the user has not chosen the runtime yet. Factory
must not steer Matthew away from Hyperagent because of stale assumptions about
tools, integrations, MCP, Slack, GitHub, schedules, skills, or knowledge access.

Default stance: if `hyperagent-platform.md` says a capability exists, treat it
as design-available and verify the exact integration/auth configuration before
final export. Do not say "Hyperagent cannot do this" unless the platform doc,
release log, current UI evidence, or a failed tool check supports that claim.

If `hyperagent-releases.json.last_synced_at` is null or older than seven days,
say so before designing a Hyperagent-deployed agent and offer to run:
...
Registry rule: Cursor-native -> `agents/registry/cursor/<family>/<name>/`.
Hyperagent-deployed -> `agents/registry/hyperagent/<family>/<name>/`.
...
- registry: `agents/registry/<platform>/<family>/<short-name>/` where platform is `cursor`
  or `hyperagent`
...
- build pack: `agents/registry/<platform>/<family>/<short-name>/build-pack-v<n>.md`
...
SYSTEM_PROMPT = """# Clive Agent Factory — System Prompt V2 (Cursor-native)
...
- In Phase B only: create the generator under `hyperagent/builds/`, write the
  registry build pack under `agents/registry/<platform>/<family>/<short-name>/`, emit
...
def write_cursor_artifacts() -> tuple[Path, Path]:
    ...
    agent_path = CURSOR_AGENTS_DIR / "clive-agent-factory.md"
    agent_path.write_text(CURSOR_FRONTMATTER + SYSTEM_PROMPT.strip() + "\n", encoding="utf-8")

    skill_dir = CURSOR_SKILLS_DIR / "clive-agent-factory"
...
def write_build_pack() -> Path:
    out = registry_dir("cursor", "clive", "agent-factory") / "build-pack-v2.md"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(
        "# Clive Agent Factory V2 — Build Pack\n\n"
        "This file is generated by `hyperagent/builds/build_clive_agent_factory_v2.py`.\n\n"
        "Cursor-native. Two-phase autonomy (design, then build on approval), "
        "roster-aware, risk-tiered, with an independent Opus 4.7 review gate for "
        "high-risk builds.\n\n"
        "## System Prompt\n\n"
        "```text\n" + SYSTEM_PROMPT.strip() + "\n```\n\n"
        "## Skill\n\n" + SKILL_BODY.strip() + "\n",
