...
Older exports (`agent-clive-curator-v0_1.json` … `v4.json`) and build scripts `build_clive_curator_v0_1.py` … `v4.py` are history. **Do not delete v0_1–v4 build scripts:** `build_clive_curator_v5.py` imports the chain.

Superseded build packs under `agents/cursor/clive/curator/` are archive-only.
