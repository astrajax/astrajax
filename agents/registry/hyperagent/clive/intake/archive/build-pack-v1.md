> **Superseded by Context Architecture V1/V2.** Live intake behaviour is governed by `clive_context_architecture_v1.md` and `clive_context_architecture_v2.md`. This build pack is historical reference only.

# Clive Intake v1 — Production Build Pack
...
**Status:** Production v1 — ready to deploy
**Last updated:** 24 May 2026
**Replaces:** `clive_intake_v0_3.md`
...
Base, table, and field IDs are populated below. Prefer field IDs on create when the integration accepts them.
...
1. ~~Create Airtable base `Clive Context OS`.~~ Done (`appYv601Oq7fKTCj0`).
2. ~~Create `Context Intake` table~~ Done (`tblJCmPGPUyszgFux`).
...
8. ~~Replace the base/table ID placeholders in the skill~~ Done.
...
13. ~~Capture all Airtable field IDs~~ Done (embedded in skill JSON).
...
## 13. Hyperagent deploy artifacts

Upload in this order:

1. **Skill:** `hyperagent/exports/skills/skill-clive-context-intake-v1.json`
2. **Agent:** `hyperagent/exports/agents/agent-clive-intake-v1.json`

After import:

1. Connect **Airtable** integration to base `appYv601Oq7fKTCj0` (AstraJax).
2. Confirm write access is limited to **Context Intake** (`tblJCmPGPUyszgFux`) if Hyperagent allows table scoping.
3. Verify all `enable*Suggestions` and `autoSave*` flags are off.
4. Run **CI-001** from section 9 in Hyperagent web chat before giving TL access.
