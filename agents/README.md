...
|---|---|---|---|
| **cursor** | Cursor subagent chat | `agents/registry/cursor/<family>/<name>/` | `.cursor/agents/<slug>.md`, `.cursor/skills/<skill>/` |
| **hyperagent** | Hyperagent web / Slack | `agents/registry/hyperagent/<family>/<name>/` | `hyperagent/exports/agents/*.json`, `hyperagent/exports/skills/*.json` |
...
`agents/registry/hyperagent/` because production is Hyperagent.

## Four artifact layers

```text
agents/registry/<platform>/<family>/<slug>/build-pack-v<n>.md   ← human spec (registry)
...
agents/
  ...
  registry/                       ← human-readable specs and build packs
...
|---|---|---|
| Registry | `agents/registry/cursor/<family>/<name>/` | `agents/registry/hyperagent/<family>/<name>/` |
...
- Design new agents with **Doc's Workshop** (`agents/registry/cursor/doc/workshop-proposer/`).
- Use `agents/draft/` for messy early designs before they become build packs.
- Workshop writes registry + runtime artifacts only after explicit Matthew approval.
