...
if (typeof window !== 'undefined') {
    ...
    window.addEventListener('unhandledrejection', (event) => {
        ...
        console.error('[email-inbox] Swallowed SDK subscriber crash');
    });
    window.addEventListener('error', (event) => {
        if (!looksLikeSDKChangeCrash(event.error)) return;
        event.preventDefault();
        console.error('[email-inbox] Swallowed SDK window error');
    });
}
    ...
    return [
        {
            ...
            label: 'Emails table',
            type: 'table' as const,
            defaultValue: base.tables.find(table => /^Emails$/i.test(table.name)),
        },
    ];
}
