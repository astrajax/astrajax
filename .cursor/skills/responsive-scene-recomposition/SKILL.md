...
description: >-
  Frame a wide painted AstraJax room for portrait and square screens without letterboxing or guillotining the subject. Compute a focal-aware cover-crop per breakpoint, emit it as a CSS object-position, re-project every hotspot into the cropped view, and flag any interactable that falls off-screen. Use when a 16:9 painted scene (Brain Vault, Doc's Workshop) must hold up on a phone, when hotspots drift off the visible slice at a breakpoint, or when planning per-device framing for a room. Triggers on phrases like "responsive scene", "portrait crop", "object-position", "hotspot off-screen on mobile", "art-directed crop". Not for cutting the layers…
---
...
Ported from Hyperagent export `skill-responsive-scene-recomposition (2).json`
(2026-08-08; preferred over `(1)` — same payload, newer stamp). Hyperagent
mechanics map to Cursor as follows:

| Hyperagent | Cursor |
|---|---|
| Skill `execute-script` / local Python | Shell: `python3 .cursor/skills/responsive-scene-recomposition/scripts/recompose.py …` |
| Image / preview files in thread | Local paths under the repo (or `/tmp`); use Read / Write |
| No credentials (`authType: none`) | No env vars required |
| Platform browser for eyeballing previews | Open the emitted `framing.png` / CSS locally; optional Playwright via `scene-dev-tools` |

**Scripts (repo-relative):**

```bash
python3 .cursor/skills/responsive-scene-recomposition/scripts/recompose.py plan \
  --manifest scene.json --out plan.json --css frame.css --image room.png --preview framing.png
python3 .cursor/skills/responsive-scene-recomposition/scripts/recompose.py verify --manifest scene.json
python3 .cursor/skills/responsive-scene-recomposition/scripts/run_test.py
# mirrors: .claude/skills/responsive-scene-recomposition/scripts/
# convenience: scripts/kate/
```

Dependencies: `python3`, `Pillow`. No `RunWithCredentials`.
...
`python3 recompose.py plan --manifest scene.json --out plan.json --css frame.css --image room.png --preview framing.png`
...
`python3 recompose.py verify --manifest scene.json`
...
Designed 5 Jul 2026 from Kate's felt gap (delivery weight was covered, but art-direction policy for wide painted scenes on portrait screens was not). Companion skills: scene-layer-cut (the hotspot anchors and layer manifest this reuses), alpha-accent-forge (accents that also need reframing), astrajax-website-map (where scene components live). Geometry follows the standard art-directed cover-crop and object-position model.
